"""future-map 直下のページを journal.emerging-future.org/future-map/ へ FTP で配る。

使い方: python3 _deploy.py <ファイル名> [--check]
...
NAME = next((a for a in sys.argv[1:] if not a.startswith("--")), "roadmaps.html")
LOCAL = Path(__file__).resolve().parent / NAME
REMOTE_DIR = "/journal.emerging-future.org/future-map"
REMOTE_FILE = f"{REMOTE_DIR}/{NAME}"
PUBLIC_URL = f"https://journal.emerging-future.org/future-map/{NAME}"
