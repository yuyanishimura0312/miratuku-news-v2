...
def main():
    ...
    p.add_argument("--out-sh", type=Path, default=BUILD / "run_expand_v2_batch.sh")
    args = p.parse_args()
    ...
    for bi, batch in enumerate(batches, 1):
        first_n = batch[0]["num"]
        last_n = batch[-1]["num"]
        prompt_path = BUILD / f"expand_v2_b{bi:02d}_ep{first_n:03d}-{last_n:03d}.md"
...
"""
        ...
        out_log = BUILD / f"expand_v2_b{bi:02d}_log.txt"
        done_marker = BUILD / f"expand_v2_b{bi:02d}_done"
        sh_lines.append(
            f'codex exec --sandbox workspace-write --skip-git-repo-check - < "{prompt_path}" > "{out_log}" 2>&1 && '
            f'touch "{done_marker}" &'
        )
        sh_lines.append(f'echo "[launched] expand_v2_b{bi:02d} ep{first_n:03d}-{last_n:03d}"')
    ...
    for bi, batch in enumerate(batches, 1):
        print(f"  expand_v2_b{bi:02d}: ep{batch[0]['num']:03d}-{batch[-1]['num']:03d} ({len(batch)} eps)")
