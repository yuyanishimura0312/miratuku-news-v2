...
### 5.1 B-1 ground truth との Mサイン階層マッピング表（B-2 sentinel verdict M1 同期）

**最終 source-of-truth は `track-b1-layered-history-report.html`** とし、B-2 DB の `msign_origin` 列はそれに同期する。Track B-3/B-4/B-5 が個別問いの Mサイン階層を参照する場合、必ず B-1 ground truth を最終確認すること（集計値ベース参照なら影響なし）。

| 問いID | B-1 ground truth | B-2 DB | analysis §211 表記 | 整合性 |
|--------|------------------|--------|--------------------|--------|
| Q-N04 | 真M由来 | 真M由来 | 真M由来 | ✓ |
| Q-N12 | 単独T由来 | 単独T由来（修正済） | 単独T由来（修正済） | ✓ |
| Q-F02 | 準M由来 | 準M由来 | 準M由来 | ✓ |
| Q-M03 | 準M由来 | 準M由来 | 準M由来 | ✓ |
| Q-V05 | 概念整合由来 | 概念整合由来 | 概念整合由来（修正済） | ✓ |
| Q-F06 | 単独T由来 | 単独T由来 | 単独T由来 | ✓ |
| Q-V01 | 単独T由来 | 単独T由来 | 単独T由来 | ✓ |

集計値（真M1 / 準M2 / 概念整合8 / 単独T3 = 14）は不変。修正は2問の個別ラベル割当のみ。
