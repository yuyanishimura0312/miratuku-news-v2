...
- 補完類型分布: 全装置応答型 **6 問** / 制度+市場 **4 問** / 研究 **6 問** / SG単独 **8 問**（全装置不応答型は実 DB で実体なし → 廃止）
- 全装置応答型 6 問: Q-N01・Q-N03・Q-N04・Q-N13・Q-M12・Q-F01
- 研究応答型（UPR強応答含む）6 問: Q-N05・Q-N12・Q-M09・Q-F08・Q-N07（科学AI）・Q-M02（AI制度反作用 mid）。Q-N07/Q-M02 は UPR で score 5 を獲得しており「全装置不応答」に該当しない（Sentinel 検証: SQL MAX(score) GROUP BY question_id で max≤2 の問いは 0 問）
...
3. **装置間補完関係に 4 類型がある**（当初設計の第 5 類型「全装置不応答型」は実 DB で実体なし → 廃止）:
   - 全装置応答型（5装置以上 score≥3）: AI制度系問い中心
   - 制度+市場応答型（Policy + IR + Funding）: 気候・教育系問い
   - 研究応答型（SG + UPR + SGRD）: 技術系問い＋Q-N07・Q-M02（UPR強応答型）
...
2. **4 補完類型分類**（第 5 類型「全装置不応答型」は廃止済み）: B-5 が Track B-3「善い社会の経路」問い × 7 装置の動き測定で、類型ごとに異なる動き解釈を採用すべき。
   - 全装置応答型問い → 「動きあり、複数経路で並走中」
   - 制度+市場応答型問い → 「動きあり、市場と政府が並走」
   - 研究応答型問い（UPR強応答型 Q-N07・Q-M02 を含む） → 「研究段階で動き、社会実装は未確立。Q-N07・Q-M02 は UPR 応答厚く大学発研究が先行」
...
本Track の 4 補完類型 のうち、第 1 類型「全装置応答型」と第 2 類型「制度+市場応答型」に該当する問いは、Track B-3「善い社会の経路」設計時に「複数経路の現実的可能性」が定量的に示せる。第 4 類型「SG単独応答型」に該当する問いは、B-3 で「現実より概念に偏る規範議論」となる留保が必要。（第 5 類型「全装置不応答型」は廃止済み）
...
2. **「装置応答 4 補完類型」分類**: 一つの問いに対する複数装置の応答パターンを 4 類型として整理する分析手法は、政府機関・大手シンクタンクのフォーサイトでは見られない独自分類（実 DB 検証に基づき当初設計の 5 類型から 4 類型に改訂）。
...
- **B-1「24 問が B-4 対象」 vs B-4「4 補完類型」**: 当初 B-1 が「装置で観測可能か」事前判断で指定した 24 問は、実 DB 集計では max_score≤2 の問いが 0 問であり、全問いに少なくとも 1 装置の強応答があった。当初設計した「第 5 類型・全装置不応答型」は実体がなく廃止。Q-N07・Q-M02 は UPR で score 5 を獲得しており、研究応答型（第 3 類型）に再分類した。
