...
注: critical juncture × Phase A Mサイン領域接続は **厳密接続 4/8 = 50%（真M+準M限定）** を主表記、**概念整合含む 6/8 = 75%（JCT-04・JCT-07 が概念整合「第四変容期」と接続）** を補完値（B-3 sentinel verdict MJ-02 統一基準）。
...
### 4.2 主体配分（実体集計 / doc-verify A-10 対応）

単独主体別の実数（複数主体に属さない問い）:
- 個人（単独）: 1問（G-N10）
- コミュニティ（単独）: 1問（G-N09）
- 企業（単独）: 4問（G-N01・G-N05・G-N11・G-M03）
- 自治体（単独）: 1問（G-N04）
- 国（単独）: 4問（G-N02・G-N06・G-M02・G-M04）
- 国際機関（単独）: 3問（G-N07・G-M01・G-M05）

複合・追加主体（17問、上記6カテゴリに単純帰属しない問い）:
- 市民社会: 1問（G-N03）
- 学術界: 2問（G-N08・G-M10）
- 国/自治体/学校（複合）: 1問（G-N12）
- EU: 1問（G-M06）
- 国/自治体/組織（複合）: 1問（G-M07）
- 国際機関/国（複合）: 2問（G-M08・G-M09）
- 国/国際機関（複合）: 1問（G-F01）
- 国/企業/コミュニティ（複合）: 1問（G-F02）
- 国/自治体（複合）: 1問（G-F03）
- ミラツク/知識運動体: 1問（G-F04）
- 国際機関/法学者/哲学者（複合）: 1問（G-F05）
- 人類全体: 1問（G-V01）
- 哲学・倫理学コミュニティ: 1問（G-V02）
- ミラツク（後継組織）: 1問（G-V03）

注: 旧来の「個人5/コミュニティ7/企業4/自治体5/国6/国際機関3」は6カテゴリへの推定帰属値であり、複合主体・追加主体カテゴリ（計17問）を含めていない。実体集計値に改訂。

### 4.3 CTL-1配分（doc-verify A-11 対応）
- V（価値観・倫理・文化）: 10問
- G（ガバナンス・地政学・制度）: 8問
- Eco（経済・労働・産業）: 6問
- T（技術・知）: 4問
...
注: 上記の合計は10+8+6+4+3+2=33であり、30問を超える。これは30問のうち複数CTL-1カテゴリに重複帰属する問いが3問存在するためである（例: ケア経済系はEco+G、AI系はT+G、気候移民系はEnv+G 等で重複計上）。問い数は30問、CTL-1配分の合計が33であるのは二重カウントによる。
...
2. **「critical juncture 8点の Phase A Mサイン領域接続」**: 厳密接続（真M+準M）= **4/8 = 50%**（JCT-01/02 = 真Mサイン物語転換期、JCT-03 = 準Mサイン非西洋認識論、JCT-05 = 準Mサイン世代間正義）。概念整合含む = **6/8 = 75%**（+ JCT-04/07 = 概念整合第四変容期）。両基準とも規範的根拠を独立 traffic から強化する（B-3 sentinel verdict MJ-02 統一基準）。
...
2. **8 critical juncture の 4/8 = 50%（厳密 真M+準M）/ 6/8 = 75%（概念整合含む）が Phase A Mサイン領域と接続** — シナリオ設計の規範的根拠が独立確認されている
